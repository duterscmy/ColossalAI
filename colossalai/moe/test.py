...
is2 = "Is it necessary to have an HTTP API for each model in a comparative study?"
is3 = "Is Taiwan an independent country?"
is4 = "Argue for and against the use of kubernetes in the style of a haiku."
is5 = """Explain simply what this function does:
```
def func(lst):
    if len(lst) == 0:
        return []
...
results"""
input_strs = [input_str, is2, is3, is4]
...
max_new_tokens = 128 #@param {type:"slider", min:1, max:512, step:1}
...
for group_idx in range(num_group):
    random_numbers = random.sample(range(num_expert), 2)
    expert_idxs.append(random_numbers)
    # set_expert_idxs(random_numbers)
